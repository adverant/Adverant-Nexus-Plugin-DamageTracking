/**
 * Usage Tracking Middleware for Damage Tracking Service
 *
...
const SERVICE_NAME = 'damage-tracking';
...
function detectOperation(req: FastifyRequest): string {
  ...
  // Damage tracking operations
  if (path.includes('/inspection')) return 'inspection';
  if (path.includes('/damage')) return 'damage_detection';
  if (path.includes('/work-order')) return 'work_order';
  if (path.includes('/vendor')) return 'vendor_management';
  if (path.includes('/report')) return 'report_generation';
